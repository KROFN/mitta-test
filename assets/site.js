...
const products=[
  ...
  },
  {
    ...
    bullets:['Секционная сборка — можно собрать и разобрать отвёрткой и ключом','Скрытые крепления, без острых углов; заглушки из натурального бука','Износостойкое порошковое покрытие','Материалы и сертификаты лучше уточнить в магазине'],
    description:'Двухъярусная металлическая кровать с секционной сборкой и жёстким изголовьем.',
    note:'* Верхнее спальное место указано как «90 × 200»; размер лучше уточнить в магазине.',
  ...
  },
  {
    ...
    description:'Спальный гарнитур с кроватью и шкафом. Рисунок фасадов можно менять перестановкой элементов.',
  ...
  },
  {
    ...
    description:'Модульная коллекция во французском стиле: решения для гостиной, кухни и спальни.',
  ...
  },
  {
    ...
    description:'Модульная гостиная из МДФ в цвете «белый глянец». Комплектуется из разных модулей.',
...
const priceBlock=(p,cls='')=>`<div class="price-block ${cls}"><strong>Цена по запросу</strong><small>Напишите — подскажем актуальную цену и наличие.</small></div>`;
...
function productCard(p,cls=''){
 if(!p.images.length)return `<a class="product-text-card ${cls}" href="/product/${p.slug}/"><strong>${p.name}</strong><b>Цена по запросу →</b></a>`;
 return `<a class="product-card ${cls}" href="/product/${p.slug}/"><div class="product-media ${p.fit==='contain'?'contain':''}">${productVisual(p)}</div><div class="product-info"><h3>${p.name}</h3><span class="price">Цена по запросу</span></div></a>`;
}
...
function home(){
 ...
 return `${header()}<main>
<section class="hero"><div class="hero-media">${img(media.hero,'Выставочный зал «Митта» с рядами мягкой мебели','',true)}</div><div class="hero-inner"><div class="hero-copy"><div class="eyebrow">Мебельный магазин · Бологое</div><h1>Мебель<br>в Бологом</h1><p class="hero-intro">Посмотрите ассортимент до поездки — а затем приезжайте в настоящий выставочный зал и выбирайте мебель вживую.</p><div class="hero-actions"><a class="btn blue" href="/catalog/">Смотреть каталог</a><a class="btn light" href="/store/">Как нас найти</a></div></div><aside class="hero-note"><b>Сначала посмотреть.</b><span>Фото зала и модели из каталога — чтобы до поездки понять, что хочется увидеть.</span></aside><div class="trust-rail"><div class="trust-item"><span class="trust-num">4,9</span><span class="trust-desc">89 оценок на Яндексе</span></div><div class="trust-item"><span class="trust-num">700+ м²</span><span class="trust-desc">выставочный зал</span></div><div class="trust-item"><span class="trust-num">с 1998</span><span class="trust-desc">ООО «Митта»</span></div></div></div></section>
...
<section class="bridge"><div class="bridge-grid"><div><div class="eyebrow">До поездки</div><h2>Сначала<br>увидеть.</h2><p>Посмотрите модели и фотографии зала, а перед визитом уточните цену и наличие конкретной позиции.</p><a class="bridge-link" href="/catalog/">Перейти к ассортименту →</a></div><div class="bridge-media">${img(media.bridge,'Кровати и матрасы в выставочном зале «Митта»')}</div></div></section>
...
function catalogCategoryCard(category,title,text,image){
 ...
 return `<a id="${category}" class="catalog-extra-card" target="_blank" rel="noopener" href="${whatsappLink(message)}"><div class="catalog-extra-media">${img(image,title)}</div><div class="catalog-extra-body"><h3>${title}</h3><span>Спросить в WhatsApp ↗</span></div></a>`;
}
...
function catalog(){
 ...
 return `${header()}<main class="catalog-page"><section class="catalog-hero"><div><div class="eyebrow">Ассортимент магазина</div><h1>Каталог</h1></div><div class="catalog-hero-side"><p>Выберите категорию или откройте модель.</p></div></section><nav class="catalog-toolbar" aria-label="Каталог"><div class="catalog-filter-group"><button class="filter-btn active" data-cat="all" aria-pressed="true">Все</button>${filterCats.map(c=>`<button class="filter-btn" data-cat="${c.key}" aria-pressed="false">${c.short}</button>`).join('')}</div><div class="catalog-view-toggle" aria-label="Размер карточек"><button type="button" class="view-btn active" data-view="compact" aria-pressed="true">Компактно</button><button type="button" class="view-btn" data-view="large" aria-pressed="false">Крупно</button></div></nav><section class="catalog-flat-wrap"><div class="catalog-flat-grid view-compact" id="catalogGrid">${visibleProducts.map(catalogCard).join('')}</div><section class="catalog-extra" id="more-directions"><div class="catalog-extra-head"><div><span class="eyebrow">Другие направления</span><h2>Кухни и дача</h2></div></div><div class="catalog-extra-grid">${catalogCategoryCard('kitchen','Кухни','',kitchen.image)}${catalogCategoryCard('garden','Дача и сад','',garden.image)}</div></section></section></main>${footer()}`;
}

function gallery(p){
 if(!p.images.length)return `<div class="pdp-no-media"><span class="eyebrow">Фото по запросу</span><strong>Запросить фото и актуальную цену</strong><p>Напишите в магазин — пришлют фото и подскажут цену и наличие.</p></div>`;
