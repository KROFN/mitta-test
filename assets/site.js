...
function productCard(p,cls=''){
 ...
 return `<a class="product-card ${cls}" href="/product/${p.slug}/"><div class="product-media ${p.fit==='contain'?'contain':''}">${productVisual(p)}</div><div class="product-info"><div><h3>${p.name}</h3><small>${p.tagline||p.cat}</small></div><span class="price">Уточнить цену</span></div></a>`;
}
function catCard(c,i){
 const hasVisual=products.some(p=>p.category===c.key&&p.images.length);
 const href=hasVisual?`/catalog/?category=${c.key}`:whatsappLink(`Здравствуйте! Интересует раздел «${c.short}». Подскажите, пожалуйста, что сейчас можно посмотреть в магазине и какие есть варианты.`);
 return `<a class="cat c${i+1}" href="${href}" ${hasVisual?'':'target="_blank" rel="noopener"'}>${img(c.image,c.name)}<div class="cat-index">${c.num}</div><div class="cat-label"><div><strong>${c.short}</strong><small>${c.desc}</small></div><span>${hasVisual?'Смотреть →':'Спросить →'}</span></div></a>`;
}
...
<section class="section products-sec home-products"><div class="home-section-head"><div><div class="eyebrow">Конкретные модели</div><h2>Выбор из каталога</h2></div><div><p class="section-lead">Пять моделей с фотографиями и ключевыми характеристиками.</p><p class="section-meta">Актуальную цену и наличие подтвердит магазин.</p></div></div><div class="product-selection">${featured.map(p=>productCard(p)).join('')}</div><div class="products-bottom"><a class="bridge-link" href="/catalog/">Смотреть весь каталог →</a></div></section>
...
function catalogCard(p){
 return `<a class="catalog-product-card" data-category="${p.category}" href="/product/${p.slug}/"><div class="catalog-product-media ${p.fit==='contain'?'contain':''}">${productVisual(p)}</div><div class="catalog-product-body"><span class="catalog-card-cat">${p.cat}</span><h3>${p.name}</h3><p>${p.tagline||p.cat}</p><div class="catalog-product-meta"><strong>Уточнить цену</strong><span>Цену и наличие подтвердим в сообщении</span></div></div></a>`;
}
function catalogTextCard(p){
 return `<a class="catalog-text-item" href="/product/${p.slug}/"><div><span class="eyebrow">Фото по запросу</span><h3>${p.name}</h3><p>${p.tagline}</p></div><div class="catalog-text-meta"><strong>Уточнить цену</strong><b>Запросить фото и цену →</b></div></a>`;
}
...
function catalog(){
 const visibleCats=cats.filter(c=>products.some(p=>p.category===c.key&&p.images.length));
 const visibleProducts=visibleCats.flatMap(c=>products.filter(p=>p.category===c.key&&p.images.length));
 const askText='Здравствуйте! Интересуют кухни или мебель для дачи. Подскажите, пожалуйста, что сейчас есть в ассортименте.';
 return `${header()}<main class="catalog-page"><section class="catalog-hero"><div><div class="eyebrow">Ассортимент магазина</div><h1>Каталог</h1></div><div><p>Модели с фотографиями и ключевыми характеристиками. Выберите раздел или просто пролистайте всё, что уже можно посмотреть на сайте.</p><p class="catalog-hero-note">Цену и наличие подтвердим в сообщении.</p></div></section><nav class="catalog-toolbar" aria-label="Категории"><button class="filter-btn active" data-cat="all" aria-pressed="true">Все</button>${visibleCats.map(c=>`<button class="filter-btn" data-cat="${c.key}" aria-pressed="false">${c.short}</button>`).join('')}</nav><section class="catalog-flat-wrap"><div class="catalog-flat-grid" id="catalogGrid">${visibleProducts.map(catalogCard).join('')}</div></section><section class="catalog-inquiry-strip"><div><span class="eyebrow">Ещё в ассортименте</span><h2>Кухни и мебель для дачи</h2><p>Напишите, что ищете — магазин подскажет доступные варианты и актуальную цену.</p></div><a class="btn blue" target="_blank" rel="noopener" href="${whatsappLink(askText)}">Спросить в WhatsApp ↗</a></section></main>${footer()}` 
}
